<template>
  <div class="main-content">
    <div class="pet-info">
      <img src="@/assets/dog.png" alt="Pet" class="pet-image">
      <h1>{{ petData.name }}</h1>
      <div class="actions">
        <button @click="performAction('feed')">Füttern</button>
        <button @click="performAction('water')">Wasser geben</button>
        <button @click="performAction('sleep')">Schlafen</button>
        <button @click="performAction('pet')">Streicheln</button>
        <button @click="performAction('clean')">Duschen</button>
        <button @click="performAction('play')">Spielen</button>
      </div>
      <transition name="fade">
        <p v-if="actionText" class="action-text">{{ actionText }}</p>
      </transition>
    </div>
    <div class="stats">
      <div class="stat-bar" v-for="(value, key) in petData" :key="key">
        <label>{{ key }}:</label>
        <div class="progress">
          <div class="progress-bar" :style="{width: value + '%' }"></div>
          <span>{{ value }}%</span>
        </div>
      </div>
    </div>
    <div class="top-pets">
      <h3>Top 10 Tiere</h3>
      <table>
        <thead>
        <tr>
          <th>#</th>
          <th>Name</th>
          <th>Tage</th>
        </tr>
        </thead>
        <tbody>
        <tr v-for="(pet, index) in pets" :key="index">
          <td>{{ index + 1 }}</td>
          <td>{{ pet.name }}</td>
          <td>{{ pet.createdDate | formatDate }}</td>
        </tr>
        </tbody>
      </table>
    </div>
  </div>
</template>

<script setup>
import { ref, onMounted } from 'vue';
import axios from 'axios';

const API_URL = 'http://localhost:8080/api/pets/top';
const pets = ref([]);
const petData = ref({

  energie: 80,
  hunger: 20,
  durst: 35,
  komfort: 75
});
const actionText = ref('');

const getTopPets = async () => {
  try {
    const response = await axios.get(API_URL);
    pets.value = response.data;
  } catch (error) {
    console.error('Error fetching top pets:', error);
  }
};

onMounted(getTopPets);

const performAction = (action) => {
  const actions = {
    feed: () => petData.value.hunger = Math.min(petData.value.hunger + 50, 100),
    water: () => petData.value.durst = Math.min(petData.value.durst + 100, 100),
    sleep: () => petData.value.energie = 100,
    pet: () => petData.value.komfort = Math.min(petData.value.komfort + 10, 100),
    clean: () => petData.value.komfort = Math.min(petData.value.komfort + 100, 100),
    play: () => {
      petData.value.energie = Math.max(petData.value.energie - 10, 0);
      petData.value.komfort = Math.min(petData.value.komfort + 10, 100);
    }
  };
  actions[action]();
  updateActionText(action);
};

const updateActionText = (action) => {
  const actionMessages = {
    feed: 'Hunger +50',
    water: 'Durst +100',
    sleep: 'Energie aufgefüllt',
    pet: 'Komfort +10',
    clean: 'Komfort +100',
    play: 'Energie -10, Komfort +10'
  };
  actionText.value = actionMessages[action];
  setTimeout(() => {
    actionText.value = '';
  }, 3000);
};

const formatDate = (date) => {
  const d = new Date(date);
  return d.toLocaleDateString();
};
</script>
<style scoped>
.main-content {
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
  padding: 20px;
  min-height: 100vh;
}

@media (min-width: 1024px) {
  .main-content {
    flex-direction: row;
    align-items: flex-start;
    justify-content: space-around;
  }

  .pet-info {
    flex: 1;
    text-align: center;
    order: 1;
  }

  .stats, .top-pets {
    flex: 1;
    order: 2;
  }
}

.pet-info {
  text-align: center;
  padding: 20px;
}

.pet-image {
  max-width: 300px;
  margin-bottom: 20px;
}

.actions {
  display: flex;
  flex-wrap: wrap;
  justify-content: center;
  gap: 10px;
  margin-bottom: 20px;
}

.action-text {
  position: fixed;
  bottom: 20px;
  left: 50%;
  transform: translateX(-50%);
  color: #fff;
  background-color: #000;
  padding: 5px 10px;
  border-radius: 5px;
  z-index: 10;
}

.stats {
  width: 100%;
  padding: 20px;
}

.stat-bar {
  width: 100%;
  margin: 10px 0;
}

.progress {
  background-color: #e0e0e0;
  width: 100%;
  height: 20px;
  border-radius: 10px;
  position: relative;
}

.progress-bar {
  background-color: #4CAF50;
  height: 100%;
  transition: width 0.3s ease;
}

.progress span {
  position: absolute;
  right: 5px;
  top: 1px;
  color: #fff;
  font-size: 14px;
}

.top-pets {
  width: 100%;
  padding: 20px;
}

table {
  width: 100%;
  border-collapse: collapse;
}

th, td {
  border: 1px solid #ddd;
  padding: 8px;
  text-align: left;
}

th {
  background-color: #333;
  color: #fff;
}
</style>